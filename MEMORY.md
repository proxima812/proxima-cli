...
- Replaced the explicit Node type requirement with a small runtime wrapper and guarded Node built-in imports so `npm publish` builds can pass even when the publish job has not installed `@types/node`.
